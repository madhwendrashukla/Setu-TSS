...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch certification logos' });
  }
});
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    const data = await prisma.certificationLogo.create({ data: req.body });
    res.json(data);
  } catch (error) {
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    const data = await prisma.certificationLogo.update({
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    await prisma.certificationLogo.delete({ where: { id: req.params.id } });
    res.json({ success: true });
  } catch (error) {
