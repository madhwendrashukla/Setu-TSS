...
router.get('/', async (req, res) => {
  try {
    const data = await prisma.incubator.findMany({
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    const data = await prisma.incubator.create({ data: req.body });
    res.json(data);
  } catch (error) {
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    const data = await prisma.incubator.update({
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    await prisma.incubator.delete({ where: { id: req.params.id } });
...
router.post('/bulk', authMiddleware, async (req, res) => {
  try {
    ...
    const data = await prisma.incubator.createMany({ data: items });
