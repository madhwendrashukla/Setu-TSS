        ...
        <section className="py-16 md:py-24 relative overflow-hidden" id="mentors">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
                <div className="text-center mb-12 md:mb-16">
                    ...
                    <p className="text-slate-300 text-base md:text-lg font-light max-w-2xl mx-auto">Three domain experts, one comprehensive system to launch your startup.</p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
                    {mentors.map((mentor, mi) => (
                        <div key={mi} className="bg-[rgba(15,23,42,0.6)] backdrop-blur-[16px] border rounded-2xl md:rounded-3xl overflow-hidden flex flex-col hover:shadow-[0_10px_30px_-10px_rgba(139,92,246,0.25)] transition-all duration-300 group hover:-translate-y-1" style={{ borderColor: `${mentor.color}40` }}>
                            ...
                            {/* Badge */}
                            <div className="inline-block px-2.5 py-0.5 text-[10px] font-bold rounded uppercase tracking-widest text-slate-50 mb-3 w-fit" style={{ background: `linear-gradient(135deg, ${mentor.color}50, ${mentor.color}30)`, border: `1px solid ${mentor.color}40` }}>
                                {mentor.workshop}
                            </div>

                            <h3 className="text-xl md:text-2xl font-extrabold text-slate-50 mb-1">{mentor.name}</h3>
                            <p className="text-sm font-bold mb-4" style={{ color: mentor.color }}>{mentor.role}</p>
                            <p className="text-sm text-slate-300 font-light leading-relaxed mb-5">{mentor.bio}</p>
                            ...
                            {/* Quote */}
                            <div className="mb-5 p-4 rounded-xl bg-white/5 border border-slate-700/50 italic text-xs text-slate-300 font-light leading-relaxed">
                                {mentor.quote}
                            </div>
