        ...
        <section className="py-16 md:py-32 relative" id="pricing">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-12 md:mb-16">
                    ...
                    <p className="text-slate-300 text-base md:text-lg font-light max-w-xl mx-auto">Individual workshops or the complete entrepreneurship bundle — you decide.</p>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6 max-w-7xl mx-auto">
                    {plans.map((plan, pi) => (
                        <div
                            key={pi}
                            className={`bg-[rgba(15,23,42,0.6)] backdrop-blur-[16px] rounded-2xl md:rounded-3xl p-6 flex flex-col transition-all duration-300 relative overflow-hidden ${plan.isFeatured ? 'border-2 border-[#8b5cf6]/50 shadow-[0_0_40px_rgba(217,70,239,0.15)] hover:shadow-[0_0_50px_rgba(217,70,239,0.25)]' : 'border border-slate-700/50 hover:border-white/25'}`}
                        >
                            {plan.isFeatured && (
                                <div className="absolute top-0 right-0 bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] text-slate-50 text-[10px] font-bold px-4 py-1.5 rounded-bl-xl uppercase tracking-widest shadow-lg">Early Bird Offer</div>
                            )}
                            {plan.saveBadge && (
                            ...
                            <div className="mb-4">
                                <div className="inline-block px-2.5 py-0.5 text-[10px] font-bold rounded uppercase tracking-widest text-slate-50 mb-2" style={{ background: plan.isFeatured ? 'linear-gradient(135deg, #8b5cf6, #d946ef)' : 'rgba(139,92,246,0.2)', border: '1px solid rgba(139,92,246,0.3)' }}>
                                    {plan.level}
                                </div>
                                <h3 className="text-lg md:text-xl font-bold text-slate-50 mb-1 leading-tight">{plan.name}</h3>
                                <p className="text-xs text-slate-300 font-light">by {plan.mentor}</p>
                            </div>
                            ...
                            <div className="mb-4">
                                ...
                                <div className={`text-4xl md:text-5xl font-extrabold ${plan.isFeatured ? 'bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] bg-clip-text text-transparent' : 'text-slate-50'}`}>
                                    {plan.earlyBird}
                                </div>
                            </div>
                            ...
                            <div className="mt-auto w-full">
                                {plan.isFeatured ? (
                                    <div className="relative w-full">
                                        <span
                                             className="relative w-full block text-center px-4 py-4 rounded-xl bg-[#1e293b] border border-slate-700/50 text-slate-50/50 font-bold transition-all text-base cursor-not-allowed"
                                         >
                                             Sold Out
                                         </span>
                                    </div>
                                    ...
                                    <span
                                        className="w-full block text-center px-4 py-3.5 rounded-xl text-slate-50/50 font-bold transition-all text-sm border border-slate-700/50 bg-[#1e293b] cursor-not-allowed"
                                    >
                                        Sold Out
                                    </span>
                                )}
