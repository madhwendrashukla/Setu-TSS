        ...
        <div>
            ...
            <div className="bg-zinc-900 border border-white/10 rounded-xl overflow-hidden">
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {startups.length === 0 ? (
                            <tr><td colSpan={5} className="p-8 text-center text-text-secondary">No startups found</td></tr>
                        ) : startups.map(s => (
                            <tr key={s.id} className="border-b border-white/5 hover:bg-white/5">
                                <td className="p-4">
                                    {s.logo_url ? (
                                        ...
                                        <div className="w-12 h-12 bg-white/10 rounded flex items-center justify-center text-xs text-white/50">None</div>
                                    )}
                                </td>
                                <td className="p-4"><p className="font-bold text-white">{s.name}</p></td>
                                ...
                                <td className="p-4"><span className={`text-xs px-2 py-1 rounded ${s.is_active ? 'text-green-400 bg-green-400/10' : 'text-white/40 bg-white/5'}`}>{s.is_active ? 'Active' : 'Hidden'}</span></td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-md p-8">
                        <h2 className="text-2xl font-bold mb-6">{editing ? 'Edit Startup' : 'Add Startup'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm text-text-secondary mb-1">Startup Name *</label>
                            ...
                            <div>
                                ...
                                <input type="url" value={formData.website_url} onChange={e => setFormData({...formData, website_url: e.target.value})} className="w-full bg-black border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-accent-blue" placeholder="https://example.com" />
                            </div>
                            <div>
                                <label className="block text-sm text-text-secondary mb-1">Logo Image</label>
                                {editing && editing.logo_url && !logoFile && (
                                    <div className="mb-2 text-xs text-white/50">Current logo: <a href={editing.logo_url} target="_blank" className="text-accent-blue underline">View</a></div>
                                )}
                                <input type="file" accept="image/*" onChange={e => setLogoFile(e.target.files?.[0] || null)} className="w-full bg-black border border-white/10 rounded px-3 py-2 text-white text-sm file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:bg-white/10 file:text-white hover:file:bg-white/20" />
                            </div>
                            <div>
