...
export default function IncubatorsManager() {
    ...
    const [formData, setFormData] = useState({ name: "", location: "", focus: "", programLength: "", equityTaken: "", website: "" });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setEditingItem(null);
                setFormData({ name: "", location: "", focus: "", programLength: "", equityTaken: "", website: "" });
                fetchData();
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button 
                    onClick={() => { setEditingItem(null); setFormData({ name: "", location: "", focus: "", programLength: "", equityTaken: "", website: "" }); setIsModalOpen(true); }}
                            ...
                            <tbody className="divide-y divide-white/10">
                                {paginatedItems.map(item => (
                                    <tr key={item.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="p-4 text-white/70">{(item.focus || []).join(", ")}</td>
                                        <td className="p-4 text-white/70">
                                            <a href={item.website} target="_blank" className="text-accent-blue hover:underline">Link</a>
                                        </td>
                                ...
                                {items.length === 0 && (
                                    <tr><td colSpan={5} className="p-8 text-center text-white/50">No incubators found.</td></tr>
                                )}
                            </tbody>
                        </table>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-[#0F1322] border border-white/10 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <h3 className="text-2xl font-bold mb-4">{editingItem ? "Edit Incubator" : "Add Incubator"}</h3>
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                            <input type="text" placeholder="Name" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required />
                            ...
                            <input type="text" placeholder="Focus Sectors (comma separated)" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" value={formData.focus} onChange={e => setFormData({...formData, focus: e.target.value})} />
                            <input type="text" placeholder="Program Length" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" value={formData.programLength} onChange={e => setFormData({...formData, programLength: e.target.value})} />
                            <input type="text" placeholder="Equity Taken" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" value={formData.equityTaken} onChange={e => setFormData({...formData, equityTaken: e.target.value})} />
                            <input type="text" placeholder="Website URL" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" value={formData.website} onChange={e => setFormData({...formData, website: e.target.value})} />
