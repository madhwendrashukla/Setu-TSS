    ...
    return (
        <>
            <button 
                ...
                className="mt-8 bg-[#A855F7] text-text-primary px-8 py-3 rounded-full font-bold transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:bg-[#9333ea]"
            >
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    {/* Backdrop */}
                    <div 
                        className="absolute inset-0 bg-accent-blue hover:bg-accent-royal text-white/40 backdrop-blur-sm transition-opacity" 
                        onClick={() => setIsOpen(false)}
                    ></div>
                    
                    {/* Modal Content */}
                    <div className="relative bg-bg-surface border border-functional-border w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <button 
                            onClick={() => setIsOpen(false)}
                            className="absolute top-6 right-6 text-text-primary/50 hover:text-text-primary transition-colors"
                        >
                            <X size={24} />
                        </button>
                        
                        <h2 className="text-3xl font-black text-text-primary mb-2 tracking-tight">Join as <span className="text-[#A855F7]">Mentor</span></h2>
                        ...
                        {status === 'success' ? (
                            <div className="bg-green-900/20 border border-green-500/30 rounded-2xl p-6 text-center">
                                ...
                                <h3 className="text-green-400 font-bold text-lg mb-2">Application Received!</h3>
                                <p className="text-green-400/80 text-sm">Thank you for your interest. Our team will review your profile and get in touch shortly.</p>
                            </div>
                        ) : (
