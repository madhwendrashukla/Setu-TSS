...
let c = fs.readFileSync('app/AI-workshop-15may/AIWorkshopFinal.tsx', 'utf-8');

// 1. Rename to DynamicFAQ and add data prop
c = c.replace(/export default function AIWorkshopFinal\(\) \{/, 'export function DynamicFAQ({ data }: { data: any }) {\n    if (!data) return null;\n    const faqs = data.faqs || [];');

// 2. Replace hardcoded FAQs map with dynamic faqs map
c = c.replace(/\{faqs\.map\(\(faq, i\) => \(/g, '{faqs.map((faq, i) => (');

// 3. Fix the dark background on the section
c = c.replace(/bg-\[\#0A0F1C\]/g, 'bg-slate-50');

// 4. Fix the dark background inside the final CTA gradient
c = c.replace(/rgba\(15,23,42,0\.95\)/g, 'rgba(255,255,255,0.95)');
c = c.replace(/border-white\/8/g, 'border-slate-200');
c = c.replace(/text-slate-900\/10/g, 'text-slate-300');

// 5. Replace first Registrations Closed button
const button1Match = /<span className="inline-flex items-center gap-3 px-12 py-5 rounded-xl font-extrabold text-slate-50\/50 text-lg bg-\[\#1e293b\] border border-slate-700\/50 cursor-not-allowed">\s*Registrations Closed\s*<\/span>/g;
...
// 6. Replace second Registrations Closed button
const button2Match = /<span\s*className="relative group w-full sm:w-auto inline-flex items-center justify-center gap-2\.5 px-10 py-4 md:px-12 md:py-5 rounded-2xl font-bold text-base md:text-lg text-slate-50\/50 bg-\[\#1e293b\] border border-slate-700\/50 cursor-not-allowed"\s*>\s*<span className="relative">Registrations Closed<\/span>\s*<\/span>/g;
